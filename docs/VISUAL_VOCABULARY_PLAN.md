...
Die fachliche Planung nutzt `OPENAI_VISUAL_PLANNER_MODEL`, ersatzweise das bereits konfigurierte `OPENAI_IMPORT_MODEL` beziehungsweise `gpt-5.6-terra`, mit mittlerem Reasoning und strengem JSON-Schema. Generierung und gezielte Bildbearbeitung sind über `OPENAI_IMAGE_GENERATION_MODEL` und `OPENAI_IMAGE_EDIT_MODEL` getrennt konfigurierbar. Beide Produktionspfade bleiben vorerst ausdrücklich auf `gpt-image-2`, weil der reale GPT-Image-2.5-Test an der Organisationsverifizierung scheiterte. Die bereits vorbereitete Trennung ermöglicht später einen kontrollierten Test von Flare für Sheets und Sunburst für Korrekturen, ohne die Produktlogik erneut umzubauen. Ohne vorhandene Ausgangsdatei fällt eine Korrektur sicher auf eine frische Generierung zurück. Ein Sheet enthält sechs Motive in einem festen Raster:
...
Jedes neue Asset speichert zusätzlich seinen Visual-Brief sowie Modell, Operation, Qualität, Größe und die zugehörige Generierungsanfrage. Sheet-Assets halten außerdem Sheetnummer, Zellindex und die verwendete `normalizationVersion` fest. Der Job protokolliert für jede erfolgreiche Images-API-Anfrage die zurückgegebenen Tokenwerte und eine Kostenschätzung anhand der im Dienst hinterlegten Preistabelle. Damit bleiben Bedeutungsentscheidung, geplanter Ausschnitt, ausgeschlossene Verwechslungen und der reale Ressourcenverbrauch bei späteren Varianten nachvollziehbar.
