...
function normalizeTeacher(entry) {
  ...
  return {
    ...
    username,
    displayName,
    role: TEACHER_ROLES.has(entry?.role) ? entry.role : "teacher",
...
class TeacherService {
  ...
  async provisionInitialPasswords({ resetPasswords = false } = {}) {
    ...
    const seededRoles = await this.loadSeededRoles();

    await this.accountStore.mutate(async (store) => {
      const timestamp = new Date().toISOString();

      for (const teacher of store.teachers) {
        const seededRole = seededRoles.get(teacher.id);
        if (seededRole && teacher.role !== seededRole) {
          teacher.role = seededRole;
          teacher.updatedAt = timestamp;
        }
  ...
  async loadSeededRoles() {
    ...
    try {
      const seed = normalizeTeacherStore(JSON.parse(await fs.promises.readFile(this.seedPath, "utf8")));
      return new Map(seed.teachers.map((teacher) => [teacher.id, teacher.role]));
    } catch (error) {
