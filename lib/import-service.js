...
class ImportService {
  ...
  async createDraft({ teacherId, text, instruction, files, purpose }) {
    ...
    const normalizedPurpose = purpose === "create_set" ? "create_set" : "append_cards";
    const shouldEnrichMetadata = normalizedPurpose === "create_set";

    if (
      deterministicDraft
      && binaryFiles.length === 0
      && !normalizedInstruction
      && (!shouldEnrichMetadata || !this.client)
    ) {
      return {
    ...
    const normalizedModelDraft = normalizeDraft(parsedDraft);
    const draft = deterministicDraft && binaryFiles.length === 0 && !normalizedInstruction
      ? { ...normalizedModelDraft, cards: deterministicDraft.cards }
      : normalizedModelDraft;
...
function buildImportInstruction({ material, teacherInstruction, shouldEnrichMetadata }) {
  const prompt = [
    ...
    "Die Vorderseite ist der abgefragte Begriff. back enthält genau eine bevorzugte Hauptantwort als erwartete Übersetzung oder Definition, keine mit Semikolon verbundene Aufzählung.",
    "acceptedAnswers enthält die Hauptantwort und jede weitere klar gleichwertige Variante aus dem Material als eigenen Array-Eintrag.",
    shouldEnrichMetadata
      ? "Erstelle außerdem passende Set-Metadaten: einen konkreten kurzen Titel, das Fach, eine knappe hilfreiche Beschreibung sowie eindeutige Bezeichnungen und Sprachcodes für Vorder- und Rückseite. Vermeide generische Titel wie 'Neues Lernset'. Leite die Angaben nur aus Material und Arbeitsauftrag ab."
