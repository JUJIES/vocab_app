...
function buildSheetPrompt(cards) {
  ...
  return [
    ...
    "Treat the six cells as unrelated flashcards. Consider only the semantic brief inside each cell; never invent a shared topic, cast, location, object set, palette, or narrative across cells.",
    "Consistency applies only to the exact grid, edge-to-edge coverage, strong legibility, and classroom-safe finish. Choose the clearest visual treatment independently for each concept: a concrete object, action, human interaction, relation, diagram, symbolic composition, or full scene.",
    "Keep each cell centered and immediately readable with simple shapes, a clear silhouette, comfortable padding, restrained detail, and a dark navy background where background is needed.",
