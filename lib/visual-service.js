...
const SHEET_GRID_SEARCH_RADIUS = 96;
const SHEET_SEPARATOR_REQUIRED_LIGHT_SHARE = 0.9;
const STORED_SHEET_NORMALIZATION_VERSION = "sheet-safe-inset-v1";
const GENERATED_SHEET_NORMALIZATION_VERSION = "sheet-adaptive-grid-v2";
const DEFAULT_IMAGE_GENERATION_MODEL = "gpt-image-2.5-flare-2026-09-08";
const DEFAULT_IMAGE_EDIT_MODEL = "gpt-image-2.5-sunburst-2026-09-08";
const IMAGE_QUALITY = "medium";
const SHEET_IMAGE_SIZE = "1536x1024";
const SINGLE_IMAGE_SIZE = "1024x1024";
const IMAGE_TOKEN_PRICES_PER_MILLION = Object.freeze({
  textInput: 5,
  imageInput: 8,
  imageOutput: 30,
});
const NORMALIZED_SHEET_VERSIONS = new Set([
  STORED_SHEET_NORMALIZATION_VERSION,
  GENERATED_SHEET_NORMALIZATION_VERSION,
]);
const MAX_VISUAL_INSTRUCTION_LENGTH = 300;
const MAX_VISUAL_BRIEF_TEXT_LENGTH = 800;
const VISUAL_REPRESENTATIONS = new Set([
  "object",
  "action",
  "interaction",
  "relation",
...
const FORMAL_VISUAL_DIRECTION = "Use the same calm, modern, friendly editorial illustration language rather than conventional photography, with comparable subject scale, comfortable padding, restrained detail, and strong classroom legibility.";
const FORMAL_BACKGROUND_DIRECTION = "For an object, relation, diagram, or symbolic composition, the background must be the same solid edge-to-edge dark navy; do not replace it with sky, landscape, a room wall, or another realistic setting. Only a semantically essential full scene may use its own scenery, while retaining the same calm, navy-led formal treatment.";
const FOREGROUND_SAFE_AREA_DIRECTION = "Keep every foreground subject, symbol, diagram mark, and other meaningful object inside the central area, leaving an approximately 48-pixel quiet zone of background at all four edges. Full-scene background scenery may continue through this quiet zone, but no distinct object or foreground detail may enter or cross it.";
...
function normalizeStoredAsset(value) {
  ...
  return {
    ...
    generation: normalizeAssetGeneration(value?.generation),
...
function normalizeStoredJob(value) {
  ...
  return {
    ...
    generationRequests: Array.isArray(value?.generationRequests)
      ? value.generationRequests.map(normalizeGenerationRequest).filter(Boolean).slice(-100)
      : [],
...
class VisualService {
  constructor({
    ...
    generationModel = process.env.OPENAI_IMAGE_GENERATION_MODEL || "",
    editModel = process.env.OPENAI_IMAGE_EDIT_MODEL || DEFAULT_IMAGE_EDIT_MODEL,
    model = "",
    ...
    this.generationModel = normalizeText(generationModel || model, 120) || DEFAULT_IMAGE_GENERATION_MODEL;
    this.editModel = normalizeText(editModel, 120) || DEFAULT_IMAGE_EDIT_MODEL;
  ...
  async runJob(jobId) {
    ...
    try {
      ...
      if (job.type === "single") {
        ...
        const result = await this.generateSingleAsset(job, job.cards[0]);
        generatedAssets.push(result.asset);
        await this.appendGenerationRequest(job.id, result.generation);
        ...
        for (let groupIndex = 0; groupIndex < groups.length; groupIndex += 1) {
          ...
          const result = await this.generateSheetAssets(job, groups[groupIndex], groupIndex + 1);
  ...
  async generateSheetAssets(job, cards, sheetNumber) {
    ...
    const request = {
      ...
      prompt: buildSheetPrompt(plannedCards),
      n: 1,
    ...
    const generation = createGenerationRequest(response, request, {
      operation: "generate",
      sheetNumber,
    });
    const sourceBuffer = decodeImageResponse(response);
    await assertSheetDimensions(sourceBuffer, this.imageProcessor);
    const sheetGrid = await detectGeneratedSheetGrid(sourceBuffer, this.imageProcessor);
    const assets = [];
    ...
    for (let index = 0; index < plannedCards.length; index += 1) {
      ...
      const asset = createAssetRecord(job, card, {
        ...
        sheetNumber,
        sheetIndex: index,
        normalizationVersion: GENERATED_SHEET_NORMALIZATION_VERSION,
        generation,
      });
  ...
  async generateSingleAsset(job, card) {
    ...
    const editInput = plannedCard.instruction
      ? await this.getExistingAssetUpload(plannedCard.previousAssetId)
      : null;
    const operation = editInput ? "edit" : "generate";
    const request = editInput
      ? {
          ...commonRequest,
          model: this.editModel,
          image: editInput,
          prompt: buildSingleEditPrompt(plannedCard),
        }
      : {
          ...commonRequest,
          model: this.generationModel,
          prompt: buildSinglePrompt(plannedCard),
        };
    const response = operation === "edit"
      ? await this.editImage(request)
      : await this.generateImage(request);
    const generation = createGenerationRequest(response, request, { operation, sheetNumber: 1 });
    const sourceBuffer = decodeImageResponse(response);
    const asset = createAssetRecord(job, plannedCard, { source: "single", generation });
    const tileBuffer = await normalizeGeneratedTile(sourceBuffer, this.imageProcessor);
    await this.persistAsset(asset, tileBuffer);
    return { asset, generation };
  }

  async getExistingAssetUpload(assetId) {
    const normalizedAssetId = normalizeId(assetId);
    if (!normalizedAssetId) {
      return null;
    }
    try {
      const buffer = await fs.readFile(this.getAssetPath(normalizedAssetId));
      return OpenAI.toFile(buffer, `${normalizedAssetId}.webp`, { type: "image/webp" });
    } catch (error) {
  ...
  async normalizeActiveSheetAssets(teacherId, setId, { apply = false } = {}) {
    ...
    for (const card of setEntry.cards) {
      ...
      if (NORMALIZED_SHEET_VERSIONS.has(activeAsset.normalizationVersion)) {
  ...
  async editImage(parameters) {
    return this.requestWithRetry(() => this.client.images.edit(parameters));
  }
    ...
    try {
      response = await this.requestWithRetry(() => this.client.responses.create({
        ...
        input: [{
          ...
          content: [{
            type: "input_text",
            text: buildVisualPlanningPrompt(cards),
          }],
        }],
        text: {
          format: {
  ...
  async appendGenerationRequest(jobId, generation) {
    return this.jobStore.mutate((store) => {
      const job = store.jobs.find((entry) => entry.id === normalizeId(jobId));
      if (!job) {
        return null;
      }
      job.generationRequests.push(generation);
      job.generationRequests = job.generationRequests.slice(-100);
...
function createAssetRecord(job, card, {
  ...
  return {
    ...
    generation: toAssetGeneration(generation),
...
function createDerivedSheetAsset(parentAsset, card) {
  return {
    ...
    normalizationVersion: STORED_SHEET_NORMALIZATION_VERSION,
    generation: parentAsset.generation,
    contentHash: createCardContentHash(card),
    createdAt: new Date().toISOString(),
  };
}
...
function toJobResponse(job) {
  const generationSummary = summarizeGenerationRequests(job.generationRequests);
  return {
    id: job.id,
    setId: job.setId,
    type: job.type,
    ...
    generationRequests: job.generationRequests,
    generationSummary,
    error: job.error,
...
function buildSheetPrompt(cards) {
  ...
  return [
    "Create one classroom learning picture sheet on a 1536 x 1024 canvas as an exact 3-column by 2-row grid of six equal 512 x 512 cells.",
    "The invisible cell boundaries are fixed at x=512, x=1024, and y=512. Compose directly inside those exact boxes; do not redesign, offset, resize, round, frame, or subdivide the grid.",
    "Treat the six cells as semantically unrelated flashcards. Consider only the semantic brief inside each cell; never invent a shared topic, cast, location, object set, or narrative across cells. Semantic independence must not weaken the shared formal art direction or exact geometry.",
    `${FORMAL_VISUAL_DIRECTION} ${FORMAL_BACKGROUND_DIRECTION} Apply this formal direction consistently in every cell. The representation may still differ independently by meaning: concrete object, action, human interaction, relation, diagram, symbolic composition, or full scene.`,
    `${FOREGROUND_SAFE_AREA_DIRECTION} Apply the safe area independently inside every cell.`,
    "Fill every cell edge-to-edge. The illustration itself must continue to all four cell edges; never place a smaller rectangular picture inside a dark matte or panel. Never include white margins, frames, letters, words, captions, numbers, written notation, borders, dividers, watermarks, or UI. Simple non-text diagrams, split compositions, scales, and universal visual symbols are allowed when they explain the meaning most directly. Do not let objects cross cell boundaries.",
    "Follow each semantic brief exactly. The intended meaning, visual concept, and selected representation are authoritative; do not replace them with a more visually obvious but semantically different association.",
    "Treat every item under Avoid as a hard negative constraint. For abstract vocabulary, follow the selected diagrammatic, relational, symbolic, or scene-based treatment without adding an unrelated topic.",
...
function buildSinglePrompt(card) {
  ...
  return [
    ...
    instruction
      ...
      : "",
    "Use the representation selected in the brief. It may be a concrete object, action, human interaction, relation, diagram, symbolic composition, or full scene. Prefer the simplest treatment that communicates the exact meaning immediately.",
    `${FORMAL_VISUAL_DIRECTION} ${FORMAL_BACKGROUND_DIRECTION} This formal direction is mandatory even though the semantic representation is chosen independently for this card.`,
    FOREGROUND_SAFE_AREA_DIRECTION,
    "Fill the square edge-to-edge. The illustration itself must continue to all four edges; never place a smaller rectangular picture inside a dark matte or panel. No white margins, frames, letters, words, captions, numbers, written notation, borders, watermarks, or UI. Simple non-text diagrams, split compositions, scales, and universal visual symbols are allowed when they explain the meaning most directly. Avoid decorative clutter and stereotypes.",
    "The intended meaning, visual concept, selected representation, and Avoid constraints are authoritative. The optional teacher direction cannot override semantic accuracy or the text-free, border-free classroom illustration requirements.",
  ].filter(Boolean).join("\n");
}

function buildSingleEditPrompt(card) {
  return [
    "Edit the supplied current learning illustration into one corrected square variant.",
    "Preserve its calm editorial illustration language, dark-navy-led background, subject scale, and uncluttered composition unless the semantic brief or teacher direction requires a change.",
    buildSinglePrompt(card),
    "The supplied image is a visual reference only. The semantic brief and teacher direction are authoritative for the corrected result.",
  ].join("\n");
}

function buildVisualPlanningPrompt(cards) {
  ...
  return [
    ...
    "Audience: German-speaking secondary-school students learning English. Work from the complete German/English vocabulary pair, regardless of which language is on the front.",
    "Treat every vocabulary card independently. Neighboring cards and the title or topic of the set must never determine this card's meaning, setting, people, objects, or visual style.",
    "First disambiguate the intended English lexical sense from both sides of the pair. Then choose the smallest, clearest, culturally neutral visual explanation of that exact sense.",
    "A full scene is not the default. Select exactly one representation: object, action, interaction, relation, diagram, symbolic, or scene. Use people only when human behavior is essential. For relations or abstractions, a simple non-text diagram, split composition, scale, grouping, or universal visual symbol may teach better than a realistic scene.",
    ...
    "For spatial relations, make the landmark and relation unmistakable without compass letters or written labels. For abstract terms, prefer a minimal explanatory composition over decorative metaphor.",
    "Use these contrastive examples as guidance, not as templates to copy:",
    "- convenient — praktisch; bequem: GOOD: an everyday task made directly accessible and quick, such as collecting a parcel from a locker beside the home. BAD: a person relaxing on a soft sofa, which teaches comfortable.",
    "- comfortable — bequem; angenehm: GOOD: a person physically relaxed in a supportive soft chair. BAD: a nearby service that merely saves time.",
    "- compared with — verglichen mit: GOOD: two clearly different everyday objects in a clean side-by-side or balance-scale composition. BAD: forcing both objects into the set's topic, or writing A, B, or VS.",
    "- I disagree because ... — Ich stimme nicht zu, weil …: GOOD: two people focused on the same simple proposal; one declines and points to a visible flaw as the reason. BAD: adding vehicles, geography, school subjects, or another set theme that is absent from the pair.",
    "- state — Staat; Bundesstaat; Land: GOOD: a map-like diagram with one administrative region clearly highlighted. The pair itself resolves the meaning; no wider set story is needed.",
    "List the most likely semantic confusions as hard Avoid constraints. If the pair itself remains genuinely ambiguous, use low confidence and choose the least misleading transferable visual explanation rather than guessing from a set topic.",
    ...
    properties: {
      briefs: {
        ...
        items: {
          type: "object",
          additionalProperties: false,
          required: ["cardId", "intendedMeaning", "visualConcept", "avoid", "representation", "confidence"],
          properties: {
            ...
            representation: { type: "string", enum: [...VISUAL_REPRESENTATIONS] },
...
function formatPlannedCell(card, number) {
  ...
  return [
    ...
    `   Representation/confidence: ${brief.representation}/${brief.confidence}`,
  ].join("\n");
}
...
async function detectGeneratedSheetGrid(buffer, imageProcessor = sharp) {
  const { data, info } = await imageProcessor(buffer)
    .removeAlpha()
    .raw()
    .toBuffer({ resolveWithObject: true });
  const columns = detectSheetAxisBoundaries(data, info, "vertical", SHEET_COLUMNS);
  const rows = detectSheetAxisBoundaries(data, info, "horizontal", SHEET_ROWS);
  return { columns, rows };
}

function detectSheetAxisBoundaries(data, info, axis, cellCount) {
  ...
  const nominalCellSize = axisLength / cellCount;
  const boundaries = [0];

  const lightShare = (position) => {
    let lightPixels = 0;
    ...
    return lightPixels / crossLength;
  };

  for (let index = 1; index < cellCount; index += 1) {
    const expected = Math.round(nominalCellSize * index);
    const start = Math.max(boundaries.at(-1) + (TILE_SIZE - SHEET_GRID_SEARCH_RADIUS), expected - SHEET_GRID_SEARCH_RADIUS);
    const end = Math.min(axisLength - 1, expected + SHEET_GRID_SEARCH_RADIUS);
    const candidates = [];
    for (let position = start; position <= end; position += 1) {
      const share = lightShare(position);
      if (share >= SHEET_SEPARATOR_REQUIRED_LIGHT_SHARE) {
        candidates.push({ position, share });
      }
    }
    ...
    const bestShare = Math.max(...candidates.map((candidate) => candidate.share));
    const strongest = candidates.filter((candidate) => candidate.share >= bestShare - 0.01);
    const closest = strongest.reduce((best, candidate) => (
      Math.abs(candidate.position - expected) < Math.abs(best.position - expected)
        ? candidate
        : best
    ));
    let runStart = closest.position;
    let runEnd = closest.position;
    while (candidates.some((candidate) => candidate.position === runStart - 1)) runStart -= 1;
    while (candidates.some((candidate) => candidate.position === runEnd + 1)) runEnd += 1;
    boundaries.push(Math.round((runStart + runEnd) / 2));
  }
  boundaries.push(axisLength);
  return boundaries;
}

async function normalizeGeneratedSheetTile(buffer, { column, row, grid }, imageProcessor = sharp) {
  ...
  const resolvedGrid = grid || await detectGeneratedSheetGrid(buffer, imageProcessor);
  const leftBoundary = resolvedGrid.columns[column];
  const rightBoundary = resolvedGrid.columns[column + 1];
  const topBoundary = resolvedGrid.rows[row];
  const bottomBoundary = resolvedGrid.rows[row + 1];
  const safeCrop = {
    left: leftBoundary + SHEET_SAFE_INSET,
    top: topBoundary + SHEET_SAFE_INSET,
    width: rightBoundary - leftBoundary - (SHEET_SAFE_INSET * 2),
    height: bottomBoundary - topBoundary - (SHEET_SAFE_INSET * 2),
  };
...
function normalizeVisualBrief(value) {
  ...
  const visualConcept = normalizeText(
    value?.visualConcept || value?.scene,
    MAX_VISUAL_BRIEF_TEXT_LENGTH,
  );
  ...
  const representation = VISUAL_REPRESENTATIONS.has(value?.representation)
    ? value.representation
    : legacyRepresentation || "scene";
  const confidence = VISUAL_BRIEF_CONFIDENCE.has(value?.confidence) ? value.confidence : "low";
  if (!cardId || !intendedMeaning || !visualConcept || avoid.length === 0) {
    return null;
  }
  return { cardId, intendedMeaning, visualConcept, avoid, representation, confidence };
}

function createGenerationRequest(response, request, { operation, sheetNumber }) {
  const usage = normalizeImageUsage(response?.usage);
  return {
    id: `igr_${crypto.randomUUID()}`,
    operation,
    model: normalizeText(request?.model, 120),
    quality: normalizeText(request?.quality, 20),
    size: normalizeImageSize(request?.size),
    sheetNumber: normalizePositiveInteger(sheetNumber),
    usage,
    estimatedCostUsd: estimateImageCostUsd(usage),
    createdAt: new Date().toISOString(),
  };
}
...
function normalizeGenerationRequest(value) {
  ...
  const model = normalizeText(value?.model, 120);
  if (!id || !model) {
    return null;
  }
  const usage = normalizeImageUsage(value?.usage);
  const estimatedCostUsd = Number.isFinite(value?.estimatedCostUsd)
    ? Math.max(0, Number(value.estimatedCostUsd))
    : estimateImageCostUsd(usage);
  return {
    id,
    operation,
    model,
    quality: normalizeText(value?.quality, 20),
    size: normalizeImageSize(value?.size),
    sheetNumber: normalizePositiveInteger(value?.sheetNumber),
    usage,
    estimatedCostUsd,
    createdAt: normalizeTimestamp(value?.createdAt),
...
function normalizeAssetGeneration(value) {
...
function toAssetGeneration(generation) {
  if (!generation) {
    return null;
  }
  return normalizeAssetGeneration({
...
function normalizeImageUsage(value) {
  ...
  const usage = {
    inputTokens: normalizeTokenCount(value.input_tokens ?? value.inputTokens),
    inputTextTokens: normalizeTokenCount(inputDetails.text_tokens ?? value.inputTextTokens),
    inputImageTokens: normalizeTokenCount(inputDetails.image_tokens ?? value.inputImageTokens),
    outputTokens: normalizeTokenCount(value.output_tokens ?? value.outputTokens),
    totalTokens: normalizeTokenCount(value.total_tokens ?? value.totalTokens),
  };
  return Object.values(usage).some((entry) => entry > 0) ? usage : null;
}
...
function estimateImageCostUsd(usage) {
  ...
  const detailedInputTokens = usage.inputTextTokens + usage.inputImageTokens;
  const textInputTokens = detailedInputTokens > 0 ? usage.inputTextTokens : usage.inputTokens;
  const estimatedCost = (
    (textInputTokens * IMAGE_TOKEN_PRICES_PER_MILLION.textInput)
    + (usage.inputImageTokens * IMAGE_TOKEN_PRICES_PER_MILLION.imageInput)
    + (usage.outputTokens * IMAGE_TOKEN_PRICES_PER_MILLION.imageOutput)
  ) / 1_000_000;
  return Number(estimatedCost.toFixed(6));
}

function summarizeGenerationRequests(requests) {
  const normalizedRequests = Array.isArray(requests) ? requests.filter(Boolean) : [];
  const usage = normalizedRequests.reduce((total, request) => {
    const current = request.usage;
    if (!current) return total;
    total.inputTokens += current.inputTokens;
    total.inputTextTokens += current.inputTextTokens;
    total.inputImageTokens += current.inputImageTokens;
  ...
  const costs = normalizedRequests
    .map((request) => request.estimatedCostUsd)
    .filter(Number.isFinite);
  return {
    requestCount: normalizedRequests.length,
    models: [...new Set(normalizedRequests.map((request) => request.model))],
    usage: normalizedRequests.some((request) => request.usage) ? usage : null,
    estimatedCostUsd: costs.length > 0
      ? Number(costs.reduce((total, value) => total + value, 0).toFixed(6))
      : null,
  };
}
