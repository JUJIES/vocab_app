...
class VisualService {
  ...
  async generateSheetAssets(job, cards, sheetNumber) {
    ...
    for (let index = 0; index < plannedCards.length; index += 1) {
      ...
      const extractedTile = await this.imageProcessor(normalizedSheet)
        .extract({
        ...
        .toBuffer();
      const tileBuffer = await normalizeGeneratedTile(extractedTile, this.imageProcessor);
...
function buildSheetPrompt(cards, setContext = {}) {
  ...
  return [
    ...
    "Fill every cell edge-to-edge with the dark navy background. Never include white margins, frames, letters, words, captions, numbers, symbols, borders, dividers, watermarks, or UI. Do not let objects cross cell boundaries.",
...
function buildSinglePrompt(card, setContext = {}) {
  ...
  return [
    ...
    "Fill the square edge-to-edge with the dark navy background. No white margins, frames, letters, words, captions, numbers, symbols, borders, watermarks, or UI. Avoid decorative clutter and stereotypes.",
...
async function normalizeGeneratedTile(buffer, imageProcessor = sharp) {
  ...
  const crop = detectLightBorderCrop(data, info);
